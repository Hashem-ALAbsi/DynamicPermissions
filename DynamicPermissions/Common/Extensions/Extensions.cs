...
namespace DynamicPermissions.Common.Extensions
{
    public static class Extensions
    {
        public static IEnumerable<TypeInfo> GetPageModels(this ApplicationPartManager appPartManager)
        {
            return appPartManager.ApplicationParts.OfType<AssemblyPart>().SelectMany(p => p.Types).Where(t=>typeof(PageModel).IsAssignableFrom(t));
        }
        public static List<AreaViewModel> GetAreaControllerActionNames(this Assembly assembly, IList<Claim>? roleClaims=null)
        {
        ...
        //public static List<AreaViewModel> GetAreaControllerActionNames(this Assembly assembly)
        //{
        //    var areaControllerActionList = assembly.GetTypes().Where(type => typeof(Controller).IsAssignableFrom(type))
        //        .SelectMany(type => type.GetMethods(BindingFlags.Instance | BindingFlags.DeclaredOnly | BindingFlags.Public))
        //        .Select(x => new
